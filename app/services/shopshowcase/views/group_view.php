...
<?php if ($filters) foreach ($filters as $filter) { ?>
  <form action="">
    <ul class="product-group__elements 	<?= $filter->name == 'Кількість секцій' ? 'sectionHeight' : '' ?>">
        ...
        <li>
          <label class="product-group__label" for="filter-<?= $filter->id ?>-<?= $value->id ?>">
            <input id="filter-<?= $filter->id ?>-<?= $value->id ?>" class="styled setInput" type="checkbox"
                   name="<?= $filter->alias ?>[]" value="<?= $value->id ?>" <?= $checked ?>>
